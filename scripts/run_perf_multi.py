...
def main() -> int:
    if len(sys.argv) > 1:
        try:
            ...
            print("usage: python scripts/run_perf_multi.py <count>", flush=True)
            return 1
    else:
        device_count = int(os.getenv("PERF_DEVICES", "2"))

    if device_count < 1:
        print("device count must be >= 1", flush=True)
        return 1
    ...
    try:
        ...
        for idx in range(1, device_count + 1):
            csv_name = f"results_device_{idx}.csv"
